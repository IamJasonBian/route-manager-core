// Book (or dry-run) an American Airlines trip on aa.com using your own
// authenticated session. Pulls credentials from Secret Manager via the auth
// loop, searches a route, selects the cheapest matching flight, and fills
// passenger details.
//
// Mirrors book-delta-trip.mjs — same safety model, AA URLs + AA_SEL_* selectors.
//
...
//
// Usage:
//   node scripts/booking/book-aa-trip.mjs --from SEA --to JFK --depart 2026-07-10 \
//        [--return 2026-07-17] [--pax 1] [--headless]
//   add --confirm-purchase only when you actually intend to buy.

import { createInterface } from 'node:readline/promises';
import { stdin, stdout } from 'node:process';
import { getAuthenticatedPage } from './aa-auth.mjs';
...
// Selectors are overridable via env — AA's booking funnel changes often. The
// defaults below are BEST-EFFORT guesses; tune them against the live site.
const SEL = {
  oneWayToggle: process.env.AA_SEL_ONEWAY || 'input[value="oneWay"], #flightSearchForm\\.tripType\\.oneWay, label:has-text("One way")',
  ...
  // Results: a list of fare cards. We pick the first/cheapest as a starting point.
  ...
  purchaseButton: process.env.AA_SEL_PURCHASE || 'button:has-text("Complete booking"), button:has-text("Purchase"), #checkout-button',
};

const SEARCH_URL = process.env.AA_SEARCH_URL || 'https://www.aa.com/booking/find-flights';
